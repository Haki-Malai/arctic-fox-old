    ...
    changePassword() {
        if (this.state.new == this.state.conf) {
            if (
                ...
                alert(
                    this.props.lang == "en"
                        ? "Password must be at least 8 characters and at most 25"
                        : "Ο κωδικός πρέπει να αποτελείται από τουλάχιστον 8 χαρακτήρες και το πολύ 25."
                );
            }
        } else {
            alert(
    ...
    render() {
        return (
            <View style={styles.optionContainer}>
                ...
                >
                    <Text style={styles.optionPressableText}>
                        {this.props.lang === "en" ? "Change" : "Αλλαγή"}
                    </Text>
                    {this.state.loading && (
                        <ActivityIndicator
